...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    foreach ($qry as $k => $v) {
        if($k=='avatar')
            $v='https://todopmbucket.s3.eu-north-1.amazonaws.com/avatars/'.(empty($v)?'profile-avatar.png':$v);
        $$k = $v;
    }
}
?>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="password" class="form-control form-control-sm"
                                   name="password" <?php echo !isset($id) ? "required" : '' ?>>
                            <small><i><?php echo isset($id) ? "Leave this blank if you dont want to change password" : '' ?></i></small>
                        </div>
                        <div class="form-group">
...
<script>
    ...
    $('#manage_user').submit(function (e) {
        ...
        const frm = $(this);
        frm.parent().find('.alert-danger').remove()
        $.ajax({
            ...
            success: function (resp) {
                ...
                if (resp == 1) {
                    ...
                    frm.prepend("<div class='alert alert-danger'>Email already exist.</div>");
                    ...
                    try {
                        ...
                        if (resp.error) {
                            frm.prepend("<div class='alert alert-danger'>" + resp.msg + "</div>");
                            $('[name="email"]').addClass("border-danger")
                        }
                    } catch (e) {
                        frm.prepend("<div class='alert alert-danger'>" + resp + "</div>");
            ...
            error: function (xhr) {
                ...
                frm.prepend("<div class='alert alert-danger'>" + xhr.responseText + "</div>");
